...
describe("Dashboard Page", () => {
  ...
  test("opens and closes Help Modal", async () => {
    ...
    const helpBtns = screen
      ...
      .filter(
        (b) =>
          b.innerHTML.includes("lucide-help") ||
          b.textContent?.includes("Help"),
      );
  ...
  test("selects all rows and opens Bulk Reminder modal", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("heading", { name: /Send Reminder/i }),
      ).toBeInTheDocument();
    });
  });
  ...
  test("filters table data securely", async () => {
    ...
    fireEvent.change(searchInput, {
      target: { value: "NonExistentTenantXYZ" },
    });
  ...
  test("adds a new record", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("heading", { name: /Add /i }),
      ).toBeInTheDocument();
    });

    const inputs = screen.getAllByRole("textbox");
    const propertyInput = inputs.find(
      (el) => el.getAttribute("name") === "property",
    );
    const tenantInput = inputs.find(
      (el) => el.getAttribute("name") === "tenantName",
    );
    const unitInput = inputs.find((el) => el.getAttribute("name") === "unit");
    const coiNameInput = inputs.find(
      (el) => el.getAttribute("name") === "coiName",
    );
    ...
    await waitFor(() => {
      expect(
        screen.queryByRole("heading", { name: /Add /i }),
  ...
  test("changes status of a record", async () => {
    ...
    const statusSelect =
      selects.find((s) => s.innerHTML.includes("Expiring Soon")) || selects[1];
